document.addEventListener('DOMContentLoaded', function() {
    ...
    async function shareResult() {
        ...
        // ボタンを一時的に無効化
        shareButton.disabled = true;
        shareButton.textContent = '共有準備中...';

        try {
            const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/png'));
            
            if (!blob) {
                throw new Error('CanvasからBlobへの変換に失敗しました。');
            }
            ...
            if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                ...
                let alertMessage = 'お使いの環境では画像共有ができません。診断結果をテキストでシェアします。';
                if (!window.isSecureContext) {
                    alertMessage += '\n\n(原因のヒント: ファイル共有機能には安全な接続(HTTPS)が必要ですが、現在のページはHTTPで接続されています。)';
                }
                alert(alertMessage);
...
※スマートフォン版ブラウザでは、結果画像を共有できます。`);
    ...
    function generateShareImage(resultData, resultType, imagePath) {
        const canvasPreviewContainer = document.getElementById('canvas-preview-container');
        canvasPreviewContainer.innerHTML = '<p>シェア画像プレビュー（縦長 4:5）</p>';
        canvasPreviewContainer.style.display = 'none';
        ...
        img.onload = () => {
            ...
            const imgBoxHeight = 700; // イラストの高さを確保
